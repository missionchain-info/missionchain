...
<p>Mission Chain utiliza el punto (.) como separador decimal internacional en todas las formulas, por ejemplo 0.65, 0.95 y 2,037,671. No sustituya estos valores por comas ni por convenciones numericas locales dentro de la notacion de formula.</p>
...
<tr><td><span class="formula-math">E<sub>base</sub> = 2,037,671 MIC/day</span></td><td>Tasa base diaria de emision derivada matematicamente. Este es el nivel neutro de emision que, mantenido durante todo el programa con su curva de decay, distribuye exactamente 5,950,000,000 MIC (85% del suministro total) al pool de mineria.</td></tr>
<tr><td><span class="formula-math">B = 6.5</span></td><td>Bootstrap multiplier activo solo en Phase I. Acelera la adopcion temprana de MICE y la siembra de liquidez sin ampliar el suministro total de MIC.</td></tr>
<tr><td><span class="formula-math">D(t) = 0.95<sup>t/90</sup></span></td><td>Funcion de time decay. La emision disminuye 5% cada 90 dias. Impone disciplina monetaria de largo plazo y escasez creciente con el tiempo.</td></tr>
<tr><td><span class="formula-math">L(t) = Clamp[(TWAP<sub>7d</sub> / L<sub>ref</sub>)<sup>α</sup>, L<sub>min</sub>, L<sub>max</sub>]</span></td><td>Liquidity factor de Hindex. Vincula la emision diaria a la profundidad real de liquidez en DEX. TWAP<sub>7d</sub> es el promedio ponderado en 7 dias de MIC/USDT. L<sub>ref</sub> es el objetivo de liquidez de referencia. α es el coeficiente de sensibilidad dentro de los limites aprobados por DAO.</td></tr>
<tr><td><span class="formula-math">R<sub>MICE,i</sub>(t) = (M<sub>i</sub> / N<sub>t</sub>) × 0.65 × E(t)</span></td><td>Recompensa diaria MIC del operador MICE i en el dia t. M<sub>i</sub> es el numero de licencias MICE activas del operador i. N<sub>t</sub> es el total de licencias MICE activas en la red. 0.65 representa el 65% de la emision diaria asignada al pool MICE.</td></tr>
...
<tr><td><span class="formula-math">PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,active</sub>) × 0.10 × E(t)</span></td><td>Asignacion diaria de MIC del pool constitucional al holder i de MFP-NFT. 0.10 representa el 10% de la emision diaria asignada al pool MFP-NFT.</td></tr>
...
<tr><td><span class="formula-math">PS<sub>i</sub>(t) = (S<sub>i</sub> / Σ S<sub>Community,active</sub>) × tier_weight × 0.10 × E(t)</span></td><td>Participacion diaria del pool Community NFT para el holder i. tier_weight es el coeficiente de ponderacion por nivel (Builder 1×, Maker 2×, Luminary 4×, salvo ajuste aprobado por governance).</td></tr>
</tbody></table></div>
...
<p>A Mission Chain usa ponto (.) como separador decimal internacional em todas as formulas, por exemplo 0.65, 0.95 e 2,037,671. Nao substitua esses valores por virgulas ou convencoes numericas locais dentro da sintaxe de formula.</p>
...
<tr><td><span class="formula-math">E<sub>base</sub> = 2,037,671 MIC/day</span></td><td>Taxa base diaria de emissao derivada matematicamente. Este e o nivel neutro de emissao que, mantido durante todo o programa com seu decay, distribui exatamente 5,950,000,000 MIC (85% do suprimento total) ao pool de mineracao.</td></tr>
<tr><td><span class="formula-math">B = 6.5</span></td><td>Bootstrap multiplier ativo apenas na Phase I. Acelera a adocao inicial de MICE e a semeadura de liquidez sem ampliar o suprimento total de MIC.</td></tr>
<tr><td><span class="formula-math">D(t) = 0.95<sup>t/90</sup></span></td><td>Funcao de time decay. A emissao diminui 5% a cada 90 dias. Impoe disciplina monetaria de longo prazo e escassez crescente ao longo do tempo.</td></tr>
<tr><td><span class="formula-math">L(t) = Clamp[(TWAP<sub>7d</sub> / L<sub>ref</sub>)<sup>α</sup>, L<sub>min</sub>, L<sub>max</sub>]</span></td><td>Liquidity factor do Hindex. Liga a emissao diaria a profundidade real de liquidez em DEX. TWAP<sub>7d</sub> e a media ponderada em 7 dias de MIC/USDT. L<sub>ref</sub> e a meta de liquidez de referencia. α e o coeficiente de sensibilidade dentro dos limites aprovados por DAO.</td></tr>
<tr><td><span class="formula-math">R<sub>MICE,i</sub>(t) = (M<sub>i</sub> / N<sub>t</sub>) × 0.65 × E(t)</span></td><td>Recompensa diaria de MIC do operador MICE i no dia t. M<sub>i</sub> e o numero de licencas MICE ativas do operador i. N<sub>t</sub> e o total de licencas MICE ativas na rede. 0.65 representa 65% da emissao diaria destinada ao pool MICE.</td></tr>
...
<tr><td><span class="formula-math">PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,active</sub>) × 0.10 × E(t)</span></td><td>Alocacao diaria de MIC do pool constitucional para o holder i de MFP-NFT. 0.10 representa 10% da emissao diaria destinada ao pool MFP-NFT.</td></tr>
...
<tr><td><span class="formula-math">PS<sub>i</sub>(t) = (S<sub>i</sub> / Σ S<sub>Community,active</sub>) × tier_weight × 0.10 × E(t)</span></td><td>Participacao diaria do pool Community NFT para o holder i. tier_weight e o coeficiente de ponderacao por nivel (Builder 1×, Maker 2×, Luminary 4×, salvo ajuste aprovado por governance).</td></tr>
</tbody></table></div>
<div class="callout gold">
...
<p>Mission Chain은 모든 수식에서 국제 표준인 점 (.) 을 소수점 구분자로 사용합니다. 예: 0.65, 0.95, 2,037,671. 수식 안에서는 쉼표나 현지 숫자 표기를 사용하지 마십시오.</p>
...
<tr><td><span class="formula-math">E<sub>base</sub> = 2,037,671 MIC/day</span></td><td>수학적으로 도출된 base daily emission rate 입니다. decay가 적용된 전체 프로그램 동안 유지되면 mining reserve에 정확히 5,950,000,000 MIC (총 공급의 85%)를 배분합니다.</td></tr>
<tr><td><span class="formula-math">B = 6.5</span></td><td>Phase I 에서만 활성화되는 bootstrap multiplier 입니다. 총 MIC supply를 늘리지 않으면서 초기 MICE adoption 과 liquidity seeding 을 가속합니다.</td></tr>
<tr><td><span class="formula-math">D(t) = 0.95<sup>t/90</sup></span></td><td>Time decay function 입니다. emission 은 90일마다 5% 감소하며 장기 통화 규율과 희소성 강화를 만듭니다.</td></tr>
<tr><td><span class="formula-math">L(t) = Clamp[(TWAP<sub>7d</sub> / L<sub>ref</sub>)<sup>α</sup>, L<sub>min</sub>, L<sub>max</sub>]</span></td><td>Hindex liquidity factor 입니다. 일일 emission 을 실제 DEX liquidity depth 에 연결합니다. TWAP<sub>7d</sub> 는 7일 MIC/USDT time-weighted average price 이고, L<sub>ref</sub> 는 reference liquidity target 입니다.</td></tr>
<tr><td><span class="formula-math">R<sub>MICE,i</sub>(t) = (M<sub>i</sub> / N<sub>t</sub>) × 0.65 × E(t)</span></td><td>day t 에서 MICE operator i 의 일일 MIC reward 입니다. M<sub>i</sub> 는 operator i 가 보유한 active MICE license 수이고, N<sub>t</sub> 는 network 전체 active MICE license 총량입니다.</td></tr>
...
<tr><td><span class="formula-math">PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,active</sub>) × 0.10 × E(t)</span></td><td>MFP-NFT holder i 가 받는 constitutional pool 의 일일 MIC allocation 입니다. 0.10 은 daily emission 중 10%가 MFP-NFT pool 에 배정됨을 뜻합니다.</td></tr>
...
<tr><td><span class="formula-math">PS<sub>i</sub>(t) = (S<sub>i</sub> / Σ S<sub>Community,active</sub>) × tier_weight × 0.10 × E(t)</span></td><td>holder i 의 Community NFT daily pool share 입니다. tier_weight 는 Builder 1×, Maker 2×, Luminary 4× 의 tier coefficient 를 의미합니다.</td></tr>
</tbody></table></div>
<div class="callout gold">
...
<p>Mission Chain su dung dau cham (.) lam dau thap phan theo chuan quoc te trong tat ca cong thuc, vi du 0.65, 0.95 va 2,037,671. Khong thay bang dau phay hoac quy uoc so dia phuong ben trong cu phap cong thuc.</p>
...
<tr><td><span class="formula-math">E<sub>base</sub> = 2,037,671 MIC/day</span></td><td>Muc phat thai co so duoc suy ra bang toan hoc. Neu duy tri trong toan bo chuong trinh cung voi decay da ap dung, muc nay se phan bo chinh xac 5,950,000,000 MIC (85% tong cung) vao mining reserve.</td></tr>
<tr><td><span class="formula-math">B = 6.5</span></td><td>Bootstrap multiplier chi hoat dong trong Phase I. He so nay tang toc do chap nhan MICE ban dau va bo sung liquidity cho he sinh thai ma khong mo rong tong cung MIC.</td></tr>
<tr><td><span class="formula-math">D(t) = 0.95<sup>t/90</sup></span></td><td>Ham time decay. Emission giam 5% sau moi 90 ngay, tao ky luat tien te dai han va muc do khan hiem tang dan theo thoi gian.</td></tr>
<tr><td><span class="formula-math">L(t) = Clamp[(TWAP<sub>7d</sub> / L<sub>ref</sub>)<sup>α</sup>, L<sub>min</sub>, L<sub>max</sub>]</span></td><td>Liquidity factor cua Hindex. Bien nay lien ket emission hang ngay voi do sau liquidity thuc te tren DEX. TWAP<sub>7d</sub> la gia MIC/USDT trung binh co trong so theo thoi gian trong 7 ngay. L<sub>ref</sub> la muc liquidity tham chieu.</td></tr>
<tr><td><span class="formula-math">R<sub>MICE,i</sub>(t) = (M<sub>i</sub> / N<sub>t</sub>) × 0.65 × E(t)</span></td><td>Phan thuong MIC hang ngay cua MICE operator i tai ngay t. M<sub>i</sub> la so license MICE dang hoat dong cua operator i. N<sub>t</sub> la tong so license MICE dang hoat dong tren toan mang. 0.65 la 65% daily emission duoc phan bo cho pool MICE.</td></tr>
...
<tr><td><span class="formula-math">PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,active</sub>) × 0.10 × E(t)</span></td><td>Phan bo MIC hang ngay tu constitutional pool cho holder MFP-NFT i. 0.10 bieu thi 10% daily emission duoc danh cho pool MFP-NFT.</td></tr>
...
<tr><td><span class="formula-math">PS<sub>i</sub>(t) = (S<sub>i</sub> / Σ S<sub>Community,active</sub>) × tier_weight × 0.10 × E(t)</span></td><td>Phan chia pool Community NFT hang ngay cho holder i. tier_weight la he so theo cap (Builder 1×, Maker 2×, Luminary 4×, tru khi governance phe duyet dieu chinh).</td></tr>
</tbody></table></div>
<div class="callout gold">
...
""".strip()
    ...
    visible_replacements = {
        "es": {
            ...
            "MIC/día": "MIC/day",
            "MIC / día": "MIC / day",
            "L(t) = Clamp[(TWAP<sub>7d</sub>(L<sub>t</sub>) / L<sub>ref</sub>)": "L(t) = Clamp[(TWAP<sub>7d</sub> / L<sub>ref</sub>)",
            ...
            "PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,activo</sub>) × 0,10 × E(t)": "PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,active</sub>) × 0.10 × E(t)",
            "R<sub>COMM,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>eff</sub>) × 0,10 × E(t)": "R<sub>COMM,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>eff</sub>) × 0.10 × E(t)",
            "R<sub>MFP,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>MFP</sub>) × 0,10 × E(t)": "R<sub>MFP,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>MFP</sub>) × 0.10 × E(t)",
            "PS_MFP_i(t) = (S_i / Σ S_MFP_active) × 0,10 × E(t)": "PS_MFP_i(t) = (S_i / Σ S_MFP_active) × 0.10 × E(t)",
        },
        "pt": {
            "× 0,65 ×": "× 0.65 ×",
            "0,95<sup>": "0.95<sup>",
            ...
            "PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,ativo</sub>) × 0,10 × E(t)": "PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,active</sub>) × 0.10 × E(t)",
            "R<sub>COMM,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>efeito</sub>) × 0,10 × E(t)": "R<sub>COMM,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>eff</sub>) × 0.10 × E(t)",
            "R<sub>MFP,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>MFP</sub>) × 0,10 × E(t)": "R<sub>MFP,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>MFP</sub>) × 0.10 × E(t)",
            "PS_MFP_i(t) = (S_i / Σ S_MFP_active) × 0,10 × E(t)": "PS_MFP_i(t) = (S_i / Σ S_MFP_active) × 0.10 × E(t)",
        },
        "ko": {
        ...
        "vi": {
            ...
            "Chia sẻ bể bơi<sub>MFP,i</sub>(t) = (S)<sub>i</sub> / Σ S<sub>MFP, đang hoạt động</sub>) × 0,10 × E(t)": "PoolShare<sub>MFP,i</sub>(t) = (S<sub>i</sub> / Σ S<sub>MFP,active</sub>) × 0.10 × E(t)",
            "R<sub>COMM,i</sub>(t) = (S)<sub>i</sub> / ΣS<sub>hiệu quả</sub>) × 0,10 × E(t)": "R<sub>COMM,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>eff</sub>) × 0.10 × E(t)",
            "R<sub>MFP,i</sub>(t) = (S)<sub>i</sub> / ΣS<sub>MFP</sub>) × 0,10 × E(t)": "R<sub>MFP,i</sub>(t) = (S<sub>i</sub> / ΣS<sub>MFP</sub>) × 0.10 × E(t)",
